...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # and C (number of channels) = nh * hs
        # e.g. in GPT-2 (124M), n_head=12, hs=64,
...
class GPTConfig:
    # max sequence length
    block_size: int = 1024
    # number of tokens: 50,000 BPE merges +
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        str_ = f"Cannot forward seq. of length {T}, b_size is only {b_size}"
        assert T <= self.config.block_size, str_
    ...
    def from_pretrained(cls, model_type):
        ...
        # copy while ensuring all of the parameters
        ...
        # but we only want to use a vanilla Linear
        # this means that we have to transpose
        # these weights when we import them
        str_ = f"mismatched keys: {len(sd_keys_hf)} != {len(sd_keys)}"
        assert len(sd_keys_hf) == len(sd_keys), str_
        for k in sd_keys_hf:
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device_type):
        ...
        # create optim groups. Any parameters
        # that is 2D will be weight decayed, otherwise no.
...
def main():
    ...
    # added after video, pytorch can be serious about
    # it's device vs. device_type distinction
    ...
    # 2. Initialize Model, Optimizer, and Loaders
